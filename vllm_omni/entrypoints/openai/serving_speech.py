...
class OmniOpenAIServingSpeech(OpenAIServing, AudioMixin):
    ...
    def _build_tts_params(self, request: OpenAICreateSpeechRequest) -> dict[str, Any]:
        ...
        if request.stream:
            params["stream"] = [True]
            params["chunk_size"] = [request.chunk_size or 5]
            params["left_context_size"] = [request.left_context_size or 25]
    ...
    async def create_speech(
        ...
        Streaming audio generation is supported via stream=true.
        """
    ...
    def _extract_chunk_audio(self, res: OmniRequestOutput):
        """Extract audio tensor, sample rate, and finished flag from a streaming chunk.

        Returns:
            Tuple of (audio_numpy, sample_rate, is_finished) or (None, None, False)
            if no audio data is available.
        """
        audio_output = None
        ...
        is_finished_tensor = audio_output.get("finished", None)
        is_finished = bool(is_finished_tensor.item()) if is_finished_tensor is not None else True
        ...
        return audio_tensor, int(sample_rate), is_finished
    ...
    async def create_speech_streaming(
        ...
        Yields audio bytes as chunks are generated by the model.
        Supports two stream formats:
        - "audio": raw encoded audio bytes per chunk
        - "sse": Server-Sent Events with base64-encoded audio data
        """
        error_check_ret = await self._check_model(request)
        if error_check_ret is not None:
            logger.error("Error with model %s", error_check_ret)
            error_bytes = json.dumps({"error": str(error_check_ret)}).encode()
            yield error_bytes
            return

        if self.engine_client.errored:
            error_bytes = json.dumps({"error": "Engine is in error state"}).encode()
            yield error_bytes
            return
        ...
        try:
            if self._is_tts_model():
                validation_error = self._validate_tts_request(request)
                if validation_error:
                    error_bytes = json.dumps({"error": validation_error}).encode()
                    yield error_bytes
                    return
            ...
            logger.info(
                "TTS streaming speech request %s: text=%r, task_type=%s",
                request_id,
                request.input[:50] + "..." if len(request.input) > 50 else request.input,
                tts_params.get("task_type", ["unknown"])[0],
            )
            ...
            async for res in generator:
                ...
                audio_numpy, sample_rate, is_finished = self._extract_chunk_audio(res)
                if audio_numpy is None:
                    continue

                # Encode audio chunk
                chunk_bytes = self.encode_audio_chunk(
                    audio_numpy, sample_rate, response_format,
                )

                if use_sse:
                    # SSE format: base64-encoded audio with metadata
            ...
            logger.info("Streaming request %s cancelled", request_id)
        except Exception as e:
