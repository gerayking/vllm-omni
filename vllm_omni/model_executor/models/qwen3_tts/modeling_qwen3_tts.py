    ...
    def _decode_worker(self):
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if context_frames > 0:
                    upsample_rate = getattr(
                        self.speech_tokenizer.model, "decode_upsample_rate", 2000
                    )
    ...
    def generate_streaming_iter(
        ...
        **kwargs,
        ...
        if suppress_tokens is None:
            suppress_tokens = [
                i
                for i in range(self.config.vocab_size - 1024, self.config.vocab_size)
        ...
        for step in range(max_new_tokens):
            ...
            if len(accumulated_codes) >= chunk_size or is_eos:
                if len(accumulated_codes) > 0:
                    ...
                    chunk_hidden = (
                        torch.stack(accumulated_hidden, dim=0) if accumulated_hidden else None
                    )
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
        ...
        **kwargs,
        ...
        with AsyncDecodingPipeline(
            ...
            for chunk_output in streaming_iter:
                ...
                pending_chunks += 1
                generation_finished = chunk_output.is_finished
    ...
    def _prepare_talker_inputs(
        ...
        # Get special embeddings - use text_projection(get_text_embeddings()) for consistency with non-streaming generate()
        tts_bos_embed, tts_eos_embed, tts_pad_embed = self.talker.text_projection(
            self.talker.get_text_embeddings()(
                torch.tensor(
                    [[self.config.tts_bos_token_id, self.config.tts_eos_token_id, self.config.tts_pad_token_id]],
