...
class Qwen3TTSModelForGeneration(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        stream = runtime_additional_information.pop("stream", [False])[0] if "stream" in runtime_additional_information else False
        chunk_size = runtime_additional_information.pop("chunk_size", [25])[0] if "chunk_size" in runtime_additional_information else 25
        left_context_size = runtime_additional_information.pop("left_context_size", [25])[0] if "left_context_size" in runtime_additional_information else 25
    ...
    def forward_streaming(
        ...
        **kwargs: Any,
        ...
        if request_id not in self._streaming_state:
            ...
            if task_type == "CustomVoice":
                generator = self.model.generate_custom_voice_streaming(
                    ...
                    chunk_size=chunk_size,
                    left_context_size=left_context_size,
                    **{k: v for k, v in kwargs.items() if k not in ["request_id", "sampling_metadata", "logits_index", "sampler"]},
                )
            elif task_type == "VoiceDesign":
        ...
        try:
            ...
            state["audio_chunks"].append(audio_chunk if isinstance(audio_chunk, np.ndarray) else audio_chunk.cpu().numpy())
    ...
    def generate_voice_design_streaming(
        ...
        **kwargs: Any,
        ...
        if not (len(texts) == len(languages) == len(instructs)):
            raise ValueError(
                f"Batch size mismatch: text={len(texts)}, "
                f"language={len(languages)}, instruct={len(instructs)}"
            )
